...
function ChatPanel({ target, onClose, history, initialMsgs, onSend }) {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState([...initialMsgs.map(m => ({ ...m, from: "them", read: true })), ...history]);
        ...
        {type && !generated && <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {[["party1","Partie 1"],["party2","Partie 2 (UID ou nom)"],["artwork","Œuvre / objet"],["price","Prix / budget"],["duration","Durée"],["commission","Commission %"]].map(([k,ph])=>(
              ...
              <input className="inp" style={styles.input} value={form[k]} onChange={e=>setForm({...form,[k]:e.target.value})} /></div>
            ))}
          </div>
...
function ReportModal({ target, onClose, onConfirm }) {
...
export default function Arcanum() {
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:DARK, minHeight:"100vh", color:"#fff" }}>
                  ...
                  {isOwn ? <WatermarkedImage color={w.color} uid={ME.uid}/> :
                    <div style={{ width:"100%", aspectRatio:"4/3", background:"#0e0e0e", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:10, position:"relative" }}>
                      ...
                      <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:8 }}>
                        ...
                        <button onClick={()=>{setChatTarget(d);setChatOpen(true);setTimeout(()=>{window.__arcanum_prefill__=`Bonjour, je souhaite accéder aux photos de « ${w.title} » (${w.artist}, ${w.year}). Pouvez-vous me les transmettre ?`;},100);}}
      ...
      {chatOpen&&chatTarget&&<ChatPanel target={chatTarget} onClose={()=>setChatOpen(false)} history={chatHistory[chatTarget.id]||[]} initialMsgs={MOCK_MSGS.filter(m=>m.fromId===chatTarget.id)} onSend={msg=>{const k=chatTarget.id;setChatHistory(h=>({...h,[k]:[...(h[k]||[]),msg]}));}}/>}
      {showInvite&&<InviteModal onClose={()=>setShowInvite(false)} onSent={uid=>{setInvitedUids(u=>[...u,uid]);toast(`Invitation envoyée · ${uid} réservé`);}}/>}
      {showContracts&&<ContractModal onClose={()=>setShowContracts(false)}/>}
      {reportTarget&&<ReportModal target={reportTarget} onClose={()=>setReportTarget(null)} onConfirm={reason=>{if(reportType==="inventory")setInventory(inv=>inv.map(i=>i.id===reportTarget.id?{...i,reports:i.reports+1,reported:true}:i));else setSearches(s=>s.map(i=>i.id===reportTarget.id?{...i,reports:i.reports+1,reported:true}:i));setReportTarget(null);toast("Signalement envoyé");}}/>}
